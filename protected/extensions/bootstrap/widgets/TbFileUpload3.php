 ...
 */
class TbFileUpload3 extends CJuiInputWidget {
     ...
     */
    public $formView = 'bootstrap.views.fileupload.form2';

    /**
     * @var string name of the upload view to be rendered
     */
    public $uploadView = 'bootstrap.views.fileupload.upload2';

    /**
     * @var string name of the download view to be rendered
     */
    public $downloadView = 'bootstrap.views.fileupload.download';
    public $listadoView = 'bootstrap.views.fileupload.listadoUpload';
    ...
    public function init() {
        if ($this->uploadTemplate === null) {
            $this->uploadTemplate = "#template-upload2";
        }
    ...
    public function run() {
        ...
        $this->htmlOptions['class'] = 'formCssClass';

        $this->options['url'] = $this->url;

        // if acceptFileTypes is not set as option, try getting it from models rules
        if (!isset($this->options['acceptFileTypes'])) {
            $fileTypes = $this->getFileValidatorProperty($this->model, $this->attribute, 'acceptFileTypes');
            if (isset($fileTypes)) {
        ...
        if (!isset($this->options['maxFileSize'])) {
            $fileSize = $this->getFileValidatorProperty($this->model, $this->attribute, 'maxFileSize');
            //$fileSize=200000000000000;
            if (isset($fileSize)) {
        ...
        // $model = new Adjunto;
        // $modelEntidadAdjunto= new EntidadAdjunto();
        ...
        //$this->render($this->listadoView,array('model'=>$model,'modelEntidadAdjunto'=>$modelEntidadAdjunto));
